...
<chocolatey xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
  ...
  context "properties" do
    ...
    context ":user" do
      ...
      it "should not allow nil" do
        expect {
          resource[:user] = nil
        }.to raise_error(Puppet::Error, /Got nil value for user/)
      end

      it "should not allow empty" do
        expect {
          resource[:user] = ''
        }.to raise_error(Puppet::ResourceError, /A non-empty user must/)
      end
    end
    ...
    context ":password" do
      ...
      it "should not allow nil" do
        ...
        }.to raise_error(Puppet::Error, /Got nil value for password/)
      end

      it "should not allow empty" do
        expect {
          resource[:password] = ''
        }.to raise_error(Puppet::ResourceError, /A non-empty password must/)
      end
    end
  end
  ...
  context ".flush" do
    ...
    it "should ensure a source is present with minimal values set" do
      PuppetX::Chocolatey::ChocolateyCommon.expects(:choco_version).returns('0.9.9.12')
    ...
    it "should set priority when choco version is exactly 0.9.9.9" do
      PuppetX::Chocolatey::ChocolateyCommon.expects(:choco_version).returns('0.9.9.9')
      Puppet::Util::Execution.expects(:execute).with([provider.command(:chocolatey),
                                                      'sources', 'add',
                                                      '--name', resource_name,
                                                      '--source', resource_location,
    ...
    it "should not set priority when choco version is less than 0.9.9.9" do
      PuppetX::Chocolatey::ChocolateyCommon.expects(:choco_version).returns('0.9.9.8')
       Puppet::Util::Execution.expects(:execute).with([provider.command(:chocolatey),
                                                      'sources', 'add',
                                                      '--name', resource_name,
                                                      '--source', resource_location,
    ...
    it "should remove a source when ensure => disabled" do
